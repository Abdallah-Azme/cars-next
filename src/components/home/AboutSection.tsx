...
export default function AboutSection() {
  ...
  const title =
    getLocalizedValue(settings, "statisticsHeading", locale) || t("title");
  const subtitle =
    getLocalizedValue(settings, "statisticsDescription", locale) ||
    t("subtitle");

  return (
    <section
      id="about-us"
      className="py-20 md:py-32 bg-slate-50 dark:bg-slate-900/50 overflow-hidden"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <div className="relative z-10 rounded-3xl overflow-hidden shadow-2xl h-[450px] md:h-[600px]">
              <Image
                src={
                  settings?.heroImage
                    ? fixImageUrl(settings.heroImage)
                    : "/about-machinery.jpg"
                }
                alt="Our Performance"
                fill
            ...
            {/* Decorative Shapes */}
            <div
              className={`absolute -bottom-10 ${isRtl ? "-right-10" : "-left-10"} w-40 h-40 bg-red-600/10 rounded-full blur-3xl z-0`}
            ></div>
            <div
              className={`absolute -top-10 ${isRtl ? "-left-10" : "-right-10"} w-60 h-60 bg-red-600/5 rounded-full blur-3xl z-0`}
            ></div>
            ...
            >
              ...
              <div className="text-start">
                <div className="text-slate-900 dark:text-white font-bold text-lg leading-tight text-nowrap">
                  {t("yearsOf")}
                </div>
          ...
          >
            ...
            <div className="mt-4 pt-4">
              <Button
                size="lg"
                className="bg-red-700 hover:bg-red-800 text-white rounded-full px-10 h-14 text-lg shadow-lg shadow-red-900/20 group"
                asChild
              >
                <Link href="/products">
                  {t("exploreFleet")}
