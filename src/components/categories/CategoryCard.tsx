...
export default function CategoryCard({category}: {category: Category}) {
  return (
    <Link href={`/categories`} className="flex flex-col gap-2 items-center group cursor-pointer">
      <div className="relative size-32 overflow-hidden border-2 border-background rounded-full flex items-center justify-center group-hover:border-red-600 transition-colors duration-500">
        <FallbackImage
          src={category?.image}
