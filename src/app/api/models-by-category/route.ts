...
/** GET /api/models-by-category?child_category_id=17 → proxies to Laravel /models?child_category_id=17 */
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(
      `${BASE_URL}/models${childCategoryId ? `?child_category_id=${childCategoryId}` : ""}`,
